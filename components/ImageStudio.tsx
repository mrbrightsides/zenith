...
const ImageStudio: React.FC<ImageStudioProps> = ({ theme, initialItem, onMounted, onInteraction }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await GeminiService.analyzeImage(base64Data, mimeType, "Analyze this image in detail. What do you see? Provide a technical breakdown.");
      setAnalysis(result);
    } catch (error) {
      console.error(error);
      ...
      <div className="flex justify-center">
        <div className="glass p-1 rounded-2xl flex gap-1">
          <button 
            ...
            className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeMode === 'generate' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:text-white'}`}
          >
            Generation
          </button>
          <button 
            onClick={() => setActiveMode('analyze')}
            className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeMode === 'analyze' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-500 hover:text-white'}`}
          >
            Analysis
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="glass p-6 rounded-3xl space-y-4 sticky top-24">
            <h2 className="text-2xl font-bold">{activeMode === 'generate' ? 'Visual Studio' : 'Neural Vision'}</h2>
            ...
            {activeMode === 'generate' ? (
              ...
              <>
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Upload Asset</label>
                    <div className="relative group">
                      ...
                      />
                      <div className={`p-8 border-2 border-dashed ${theme === 'light' ? 'border-slate-300 bg-slate-100' : 'border-slate-800 bg-slate-900/50'} rounded-2xl flex flex-col items-center justify-center gap-2 group-hover:border-indigo-500 transition-colors`}>
                        <i className="fas fa-cloud-upload-alt text-2xl text-slate-500"></i>
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Drop Image or Click</span>
                      </div>
                    </div>
                  </div>

                  {previewUrl && (
                    <div className="glass p-2 rounded-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                  ...
                  <button
                    ...
                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20 transition-all active:scale-95"
                  >
                    {analyzing ? <i className="fas fa-circle-notch fa-spin"></i> : <i className="fas fa-microscope"></i>}
                    Analyze Visual Data
                  </button>
                </div>
              </>
        ...
        <div className="lg:col-span-2 space-y-8">
          {activeMode === 'generate' ? (
            ...
            <>
              {analysis ? (
                <div className="glass p-8 rounded-[2.5rem] border border-indigo-500/20 bg-indigo-500/5 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                    ...
                    <div>
                      <h3 className="text-xl font-bold tracking-tight">Neural Analysis Report</h3>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Gemini Vision Core v1.5</p>
                    </div>
                  </div>
                  <div className="prose prose-invert prose-sm max-w-none">
                    <div className="text-slate-300 leading-relaxed whitespace-pre-wrap font-mono text-[11px]">
                      {analysis}
                    </div>
                  </div>
                </div>
                ...
                <div className={`h-[500px] border-2 border-dashed ${theme === 'light' ? 'border-slate-300' : 'border-slate-800'} rounded-3xl flex flex-col items-center justify-center text-slate-500 gap-4`}>
                  <i className="fas fa-microscope text-6xl opacity-10"></i>
                  <p className="font-medium opacity-50 uppercase tracking-widest text-xs">Awaiting Visual Input</p>
                </div>
              )}
            </>
          )}

          {activeMode === 'generate' && history.length > 0 && (
            <div className="space-y-4">
